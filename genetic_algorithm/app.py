...
@click.option('-v', '--volume', type=float, help='Maximum volume of the packages that can be chosen')
@click.option('-w', '--weight', type=float, help='Maximum weight of the packages that can be chosen')
@click.option('-d', '--distance', type=float, help='Maximum distance that can be travelled')
@click.option('-mp', '--min_packs', type=int, help='Minimum number of selected packs')
@click.option('-x', '--address_x', type=float, help='X coordinate of the starting address')
@click.option('-y', '--address_y', type=float, help='Y coordinate of the starting address')
@click.option('-f', '--file', type=str, help='Path to the input file', required=True, prompt=True)
@click.option('-p', '--population', type=int, default=1500, help='Size of the population')
@click.option('-g', '--generations', type=int, default=200, help='Maximum number of iterations')
@click.option('-ni', '--no_improve', type=int, default=30, help='Maximum number of iterations without improvement')
@click.option('-a', '--alpha', type=float, default=2.0, help='Parameter of the objective function')
...
@click.option('-cf', '--crossover_function', type=click.Choice(list(CROSS_DICT.keys())), default=["random"], multiple=True, help='Crossover function')
@click.option('-mf', '--mutation_function', type=click.Choice(list(MUTATION_DICT.keys())), default=["random"], multiple=True, help='Mutation function')
@click.option('-er', '--elitism_rate', type=float, default=0.1, help='Elitism rate')
@click.option('-cma', '--crossover_max_attempts', type=int, default=10, help='Maximum number of attempts to perform crossover')
@click.option('-mma', '--mutation_max_attempts', type=int, default=10, help='Maximum number of attempts to perform mutation')
@click.option('-le', '--log_every', type=int, default=1, help='Log info every x generations')
@click.option('-bf', '--brute_force', is_flag=True, default=False, show_default=True, help='Run brute force instead of genetic algorithm')
def cli(**kwargs):
    """Run the CLI"""
    ...
    with open(kwargs['file'], 'r') as f:
        ...
        while line:
            ...
            packs.append(Package(int(index), float(weight), float(volume), (float(x), float(y)), int(priority)))
            line = f.readline()
    ...
    if 'random' in crossover_functions:
        ...
        crossover_function = [CROSS_DICT[crossover] for crossover in crossover_functions]
