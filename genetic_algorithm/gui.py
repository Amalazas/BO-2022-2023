...
class GUI:
    def __init__(self):
        ...
        max_volume_entry.grid(row=0, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")
        ...
        max_weight_entry.grid(row=1, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")
        ...
        max_distance_entry.grid(row=2, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")
        ...
        min_chosen_packs_entry = ctk.CTkEntry(self.root, textvariable=self.min_chosen_packs_var)
        min_chosen_packs_entry.grid(row=3, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

        # Start Address Input
        start_address_label = ctk.CTkLabel(self.root, text="Start Address X Y")
        start_address_label.grid(row=4, column=0, padx=5, pady=5)
        start_address_x_entry = ctk.CTkEntry(self.root, textvariable=self.start_address_x_var)
        start_address_x_entry.grid(row=4, column=1, padx=5, pady=5)
        start_address_y_entry = ctk.CTkEntry(self.root, textvariable=self.start_address_y_var)
        ...
        population_size_entry = ctk.CTkEntry(self.root, textvariable=self.population_size_var)
        population_size_entry.grid(row=5, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")
        ...
        max_generations_entry = ctk.CTkEntry(self.root, textvariable=self.max_generations_var)
        max_generations_entry.grid(row=6, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")
        ...
        alpha_entry.grid(row=7, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

        # Mutation Rate Input
        mutation_rate_label = ctk.CTkLabel(self.root, text="Mutation Rate")
        mutation_rate_label.grid(row=8, column=0, padx=5, pady=5)
        mutation_rate_entry = ctk.CTkEntry(self.root, textvariable=self.mutation_rate_var)
        mutation_rate_entry.grid(row=8, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

        # Elitism Rate Input
        elitism_rate_label = ctk.CTkLabel(self.root, text="Elitism Rate")
        elitism_rate_label.grid(row=9, column=0, padx=5, pady=5)
        elitism_rate_entry = ctk.CTkEntry(self.root, textvariable=self.elitism_rate_var)
        elitism_rate_entry.grid(row=9, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

        # Crossover Max Attempts Input
        crossover_max_attempts_label = ctk.CTkLabel(self.root, text="Crossover Max Attempts")
        crossover_max_attempts_label.grid(row=10, column=0, padx=5, pady=5)
        crossover_max_attempts_entry = ctk.CTkEntry(self.root, textvariable=self.crossover_max_attempts_var)
        crossover_max_attempts_entry.grid(row=10, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

        # Mutation Max Attempts Input
        mutation_max_attempts_label = ctk.CTkLabel(self.root, text="Mutation Max Attempts")
        mutation_max_attempts_label.grid(row=11, column=0, padx=5, pady=5)
        mutation_max_attempts_entry = ctk.CTkEntry(self.root, textvariable=self.mutation_max_attempts_var)
        mutation_max_attempts_entry.grid(row=11, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

        # Log Every Input
        log_every_label = ctk.CTkLabel(self.root, text="Log Every")
        log_every_label.grid(row=12, column=0, padx=5, pady=5)
        log_every_entry = ctk.CTkEntry(self.root, textvariable=self.log_every_var)
        log_every_entry.grid(row=12, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

        # Crossover Function dropdown
        crossover_function_label = ctk.CTkLabel(self.root, text="Crossover Function")
        crossover_function_label.grid(row=13, column=0, padx=5, pady=5)
        self.crossover_function_label_combo = ctk.CTkComboBox(self.root, values=list(CROSS_DICT.keys()))
        self.crossover_function_label_combo.grid(row=13, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

        # Mutation Function dropdown
        mutation_function_label = ctk.CTkLabel(self.root, text="Mutation Function")
        mutation_function_label.grid(row=14, column=0, padx=5, pady=5)
        self.mutation_function_label_combo = ctk.CTkComboBox(self.root, values=list(MUTATION_DICT.keys()))
        self.mutation_function_label_combo.grid(row=14, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

        # Packs File Input
        packs_file_label = ctk.CTkLabel(self.root, text="Packs File")
        packs_file_label.grid(row=15, column=0, padx=5, pady=5)
        packs_file_button = ctk.CTkButton(self.root, text="Select File", command=self._select_file)
        packs_file_button.grid(row=15, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")
        self.packs_text = ctk.CTkTextbox(self.root)
        self.packs_text.grid(row=16, column=0, padx=5, pady=5, sticky="nsew", columnspan=3)

        # Run Button
        self.run_button = ctk.CTkButton(self.root, text="Run", command=self._run_simulation)
        self.run_button.grid(row=17, column=0, padx=5, pady=5, columnspan=3, sticky="nsew")
    ...
    def _run_simulation(self):
        if self.file_chosen:
            ...
            self.ga = GeneticAlgorithm(
                ...
                start_address=(self.start_address_x_var.get(), self.start_address_y_var.get()),
                packs=self.packs,
                ...
                crossover_function=CROSS_DICT[self.crossover_function_label_combo.get()],
                mutation_function=MUTATION_DICT[self.mutation_function_label_combo.get()],
            )
            self.root.iconify()
            self.ga.run()

            self.results_frame = ctk.CTkFrame(self.root, width=600)
            self.results_frame.grid(row=0, column=4, padx=5, pady=5, sticky="nsew", columnspan=3, rowspan=18)
            ...
            canvas1.draw()
            canvas1.get_tk_widget().grid(row=0, column=0, padx=5, pady=5, sticky="nsew", rowspan=1)
            canvas2 = FigureCanvasTkAgg(fig2, master=self.results_frame)
            canvas2.draw()
            canvas2.get_tk_widget().grid(row=1, column=0, padx=5, pady=5, sticky="nsew", rowspan=1)
            self.root.deiconify()
